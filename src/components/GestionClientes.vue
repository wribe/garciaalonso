<!--<template>
  <div class="mx-auto my-1 mt-2 p-3 pb-5 border rounded-3 shadow-sm min-vh-75 bg-light">
...
<!-- Apellidos 
      ...
      <div class="mb-3 row g-3 align-items-center">
        ...
        <div class="col-md-5 d-flex align-items-center">
          ...
          <input type="text" id="direccion" v-model="nuevoCliente.direccion" @blur="capitalizarTexto('direccion')"
      ...
      <div class="d-flex align-items-center mt-3">
        ...
        <div class="flex-grow-1 d-flex justify-content-center">
          <button type="submit" class="btn btn-primary ms-5 px-3 btn-sm">
    ...
    <div class="table-responsive">
      ...
      <table class="table table-bordered table-striped table-hover table-sm aling-middle">
        ...
        <tbody>
          <tr v-for="(cliente, index) in clientesPaginados" :key="cliente.id || index">
            <th scope="row" class="text-center">{{ (currentPage -1) * clientePorPage + index + 1 }}</th>
            ...
            <td class="align-middle text-center">
              <button @click="eliminarCliente(cliente.movil)"
                class="btn btn-danger btn-sm me-2 boder-0 ms-4 shadow-none rounded-0">
                <i class="bi bi-trash"></i>
              </button>
              <button @click="editarCliente(cliente.movil)"
                class="btn btn-warning btn-sm border-0 shadow-none rounded-0" title="Editar cliente"
...
//Declarion de estado o variables reactivas
const editando = ref(false);
const clienteEditandoId = ref(null);
const numClientes = ref(0);
const mostrarHistorico = ref(false);
const currentPage = ref(1);
const clientePorPage = 10;
...
//es decir, paso página o vuelvo atrás cargando los clientes de esa página
//slice estrae una sección de array de clienrtes
  ...
  <div class="mx-auto mt-2 p-3 pb-5 border rounded-3 shadow-sm min-vh-75 bg-light">
    ...
    <form @submit.prevent="guardarCliente" class="mb-4">
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-3 d-flex align-items-center">
          <label class="me-2 mb-0 text-nowrap align-middle" for="">Contraseña:</label>
          <input type="password" v-model="nuevoCliente.password" class="form-control" :class="{ 'is-invalid': !contrasenaValida }"
            @blur="validarContrasena" required>
        </div>

        <div class="col-md-3 d-flex mt-3 align-items-center ms-5">
          <label class="me-2 mb-0 text-nowrap align-middle" for="">Confirmar Contraseña:</label>
          <input type="password" v-model="nuevoCliente.password2" class="form-control" :class="{ 'is-invalid': !contrasenaValida }"
            @blur="validarContrasena" required>
        </div>
      </div>
      </div>
...
const clienteEditandoId = ref(null);
const mostrarHistorico = ref(false); // Control del estado del checkbox
...
const usuario = localStorage.getItem("isUsusario") === "true";
...
const validarContrasena = () => {
  const contrasena = nuevoCliente.value.password.trim();
  const repetir = password2.value.trim();
...
const buscarClientePorDNI = async (dni) => {
  ...
  try {
    ...
    editando.value = true;
    clienteEditandoId.value = cliente.id;
    nuevoCliente.value = cliente.value;
